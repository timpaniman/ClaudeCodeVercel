    ...
    <li>
      ...
      >
        <div className="flex items-start gap-2">
          {!item.read && (
            <span className="mt-2 h-2.5 w-2.5 shrink-0 rounded-full bg-indigo-400" role="img" aria-label="안 읽음" data-testid="unread-dot" />
          )}
          <div className="min-w-0 flex-1 space-y-1.5">
            <h3 className={cn('text-base leading-snug break-words', item.read ? 'font-medium text-gray-200' : 'font-semibold text-white')}>
              {item.isPinned && <Pin size={14} className="mr-1.5 inline -mt-0.5 text-indigo-300" aria-label="고정 공지" />}
              {item.title}
            </h3>
